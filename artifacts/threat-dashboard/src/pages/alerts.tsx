...
import {
  ...
  User, Clock, FileX, AlertTriangle,
} from "lucide-react";
...
const TYPE_CONFIG: Record<BlocklistType, { label: string; icon: React.ReactNode; color: string; bg: string }> = {
  ip: {
    label: "Blocked IP",
...
function TypeBadge({ type }: { type: BlocklistType }) {
  ...
  return (
    <Badge variant="outline" className={`font-mono text-[10px] uppercase flex items-center gap-1 ${cfg.bg} ${cfg.color} border`}>
      {cfg.icon}
      {cfg.label}
    </Badge>
  );
...
function AddEntryDialog({ onAdded }: { onAdded: () => void }) {
  ...
  const [form, setForm] = useState({ type: "ip" as BlocklistType, value: "", reason: "", notes: "" });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setOpen(false);
      setForm({ type: "ip", value: "", reason: "", notes: "" });
      onAdded();
    } catch (e: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="font-mono text-xs uppercase tracking-widest shrink-0">
          <Plus className="w-4 h-4 mr-2" /> Block Indicator
        </Button>
      </DialogTrigger>
      <DialogContent className="bg-card border-border sm:max-w-[440px]">
        <DialogHeader>
          <DialogTitle className="font-mono uppercase tracking-widest text-base flex items-center gap-2">
            <ShieldOff className="w-4 h-4 text-destructive" /> Add to Blocklist
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-4 mt-2">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              className="font-mono bg-background/50"
              placeholder="e.g. C2 server, malware dropper, phishing..."
              value={form.reason}
...
export default function Alerts() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed");
      toast({ title: "Entry removed from blocklist" });
      fetchEntries();
    } catch {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col h-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-muted-foreground font-mono tracking-widest">NETWORK SECURITY BLOCKLIST</p>
        </div>
        {canEdit && <AddEntryDialog onAdded={fetchEntries} />}
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Input
          placeholder="Search by value, reason or analyst..."
          value={search}
          onChange={e => setSearch(e.target.value)}
          className="font-mono bg-background/50 border-border/50 text-sm sm:max-w-xs"
        />
      </div>

      {/* List */}
      <div className="flex-1 min-h-0 space-y-3 overflow-auto pb-12">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-48 bg-card/20 rounded-lg border border-dashed border-border/50 gap-3">
            ...
            {!search && canEdit && <p className="text-xs text-muted-foreground font-mono">Click "Block Indicator" to add an entry</p>}
          </div>
        ) : (
          filtered.map((entry, i) => {
            ...
            return (
              <Card
                ...
                style={{
                  borderLeftColor: entry.type === "ip" ? "hsl(var(--destructive))" :
                    entry.type === "hash" ? "#fb923c" :
                    entry.type === "domain" ? "hsl(var(--warning))" : "#a78bfa",
